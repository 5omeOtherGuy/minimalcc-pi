...
- `tool-json-arguments.ts` — pure best-effort repair/parse of partial Anthropic `tool_use` input JSON fragments into a plain arguments record (escapes raw control chars, preserves/rewrites string escapes, closes truncated strings/containers); always returns an object so tool calls never surface invalid arguments.
