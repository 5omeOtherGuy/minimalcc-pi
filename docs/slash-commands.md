...
| `/claude-subscription-usage` | Per-process token, cache, and request totals for traffic routed through this extension. Append `reset` to clear them. |
| `/claude-subscription-cache-diagnostics` | Per-process record of cache-read drops between comparable requests, with a fingerprint of which request-shape section changed. Append `reset` to clear them. |
| `/claude-subscription-microcompaction` | Per-process keep-recent microcompaction counters (requests, applied requests, cleared tool results, bytes saved). Append `reset` to clear them. |
...
Reset: run `/claude-subscription-usage reset` to clear the per-process counters (including the persistent aggregate totals that survive sample eviction) without restarting Pi. It replies with a single info notification (`Claude subscription usage telemetry reset for this process.`) and reports no counters. Any argument other than `reset` is ignored and the normal summary is shown.
...
Reset: run `/claude-subscription-cache-diagnostics reset` to clear recorded events and per-session comparison state without restarting Pi. It replies with a single info notification (`Claude subscription cache diagnostics reset for this process.`). The fingerprint salt is fixed for the process lifetime and is not regenerated by reset. Any argument other than `reset` is ignored and the normal summary is shown.

## `/claude-subscription-microcompaction`

How to use: run after `claude-subscription` requests have completed in the current Pi process **with microcompaction enabled** (`PI_CLAUDE_MICROCOMPACT` truthy). When the feature is disabled, no requests are recorded and the counters stay at zero. It does **not** capture prompt text, tool arguments, file paths, model output, or credentials.

Microcompaction is opt-in and changes only what the request sends to Anthropic — old, large, text-only, non-error tool results are replaced in place with `[Old tool result content cleared]` while the most recent results stay full. The Pi session transcript is never modified.

Environment knobs (read per request):

- `PI_CLAUDE_MICROCOMPACT` — enable when set to `1`/`true`/`yes`/`on`. Default OFF.
- `PI_CLAUDE_MICROCOMPACT_KEEP_RECENT` — number of most-recent clearable tool results kept full. Default `5`, floored to `1`.
- `PI_CLAUDE_MICROCOMPACT_MIN_BYTES` — only compact when it frees at least this many bytes (`Buffer.byteLength`). Default `65536`.

Output (single info notification; one line, current process counters):

```
Claude subscription microcompaction: requests=N applied=N compactedResults=N bytesSaved=N skippedIncomplete=N
```

Fields:

- `requests` — requests evaluated while microcompaction was enabled (disabled requests are not counted).
- `applied` — requests where at least one tool result was cleared (the byte gate passed).
- `compactedResults` — total tool results cleared across all requests.
- `bytesSaved` — total UTF-8 bytes of tool-result text reclaimed (net of the placeholder).
- `skippedIncomplete` — tool results skipped because they were orphan/incomplete sequences not sent to Anthropic.

How to interpret it:

- When microcompaction first clears an old result, the separate `/claude-subscription-cache-diagnostics` command may independently report a `messages` change / cache-read drop. That is expected: the two stores are intentionally not wired together.
- All counters are per-process and reset on Pi restart.

Reset: run `/claude-subscription-microcompaction reset` to clear the per-process counters without restarting Pi. It replies with a single info notification (`Claude subscription microcompaction telemetry reset for this process.`). Any argument other than `reset` is ignored and the normal summary is shown.
