...
- `src/native-usage-telemetry.ts` records in-process input/output/cache-read/cache-write/total-token totals per `claude-subscription` response, retains only the latest bounded sample records, and renders the redacted summary surfaced by `/claude-subscription-usage`.
